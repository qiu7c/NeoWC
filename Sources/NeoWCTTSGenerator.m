...
static NSMutableSet<NeoWCTTSRequest *> *NeoWCTTSActiveRequests(void) {
    static NSMutableSet<NeoWCTTSRequest *> *requests;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{ requests = [NSMutableSet set]; });
    return requests;
}

static NSError *NeoWCTTSError(NSInteger code, NSString *message) {
    return [NSError errorWithDomain:NeoWCTTSErrorDomain code:code
                           userInfo:@{NSLocalizedDescriptionKey: message ?: @"语音生成失败"}];
}

@implementation NeoWCTTSRequest
...
- (void)finishWithError:(NSError *)error {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        @synchronized (NeoWCTTSActiveRequests()) {
            [NeoWCTTSActiveRequests() removeObject:self];
        }
...
- (void)startWithText:(NSString *)text {
    ...
    if (![NSFileManager.defaultManager createDirectoryAtPath:directory
        ...
        [self finishWithError:directoryError ?: NeoWCTTSError(1, @"无法创建 TTS 临时目录")];
        return;
    }
    ...
    [self.synthesizer writeUtterance:utterance toBufferCallback:^(AVAudioBuffer *buffer) {
        ...
        if (writeFailed) {
            [strongSelf finishWithError:writeError ?: NeoWCTTSError(4, @"无法写入生成的语音")];
        }
    }];
}
...
void NeoWCGenerateLocalSpeech(NSString *text,
                              void (^completion)(NSURL *outputURL, NSError *error)) {
    NSString *trimmed = [text stringByTrimmingCharactersInSet:
        NSCharacterSet.whitespaceAndNewlineCharacterSet];
    if (trimmed.length == 0) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (completion) completion(nil, NeoWCTTSError(5, @"请输入需要合成的文字"));
        });
        return;
    }
    if (trimmed.length > 500) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (completion) completion(nil, NeoWCTTSError(6, @"首版 TTS 最多支持 500 个字符"));
        });
        return;
    }
    dispatch_async(dispatch_get_main_queue(), ^{
        NeoWCTTSRequest *request = [NeoWCTTSRequest new];
        request.completion = completion;
        @synchronized (NeoWCTTSActiveRequests()) {
            [NeoWCTTSActiveRequests() addObject:request];
        }
