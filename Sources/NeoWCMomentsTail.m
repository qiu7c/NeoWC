...
static BOOL NeoWCMomentsTailPostSessionSet;
static NSString *NeoWCMomentsTailPostSessionAppID;

static NSObject *NeoWCMomentsTailStateLock(void) {
...
static NSArray<NSDictionary<NSString *, NSString *> *> *NeoWCMomentsTailEntries(void) {
...
static NSString *NeoWCMomentsTailNameForAppID(NSString *appID) {
    if (appID.length == 0) return nil;
    for (NSDictionary *entry in NeoWCMomentsTailEntries()) {
        if ([entry[@"id"] isEqualToString:appID]) return entry[@"name"];
    }
    return nil;
}

static NSString *NeoWCMomentsTailEffectiveAppID(void) {
    if (![NSUserDefaults.standardUserDefaults boolForKey:NeoWCMomentsTailEnabledKey]) return nil;
    @synchronized (NeoWCMomentsTailStateLock()) {
        if (NeoWCMomentsTailPostSessionSet) return NeoWCMomentsTailPostSessionAppID;
    }
    return [NSUserDefaults.standardUserDefaults stringForKey:NeoWCMomentsTailAppIDKey];
}

static NSString *NeoWCMomentsTailPickerSelection(BOOL postSessionMode) {
    if (postSessionMode) {
        @synchronized (NeoWCMomentsTailStateLock()) {
            if (NeoWCMomentsTailPostSessionSet) return NeoWCMomentsTailPostSessionAppID;
        }
...
static void NeoWCMomentsTailResetPostSession(void) {
    @synchronized (NeoWCMomentsTailStateLock()) {
        NeoWCMomentsTailPostSessionSet = NO;
        NeoWCMomentsTailPostSessionAppID = nil;
    }
}

#pragma mark - Picker

@interface NeoWCMomentsTailPickerViewController : UITableViewController <UISearchResultsUpdating>
...
@implementation NeoWCMomentsTailPickerViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = self.postSessionMode ? @"发圈尾巴" : @"朋友圈小尾巴";
    self.tableView.backgroundColor = UIColor.systemGroupedBackgroundColor;
    self.displayedEntries = NeoWCMomentsTailEntries();
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *selectedID = NeoWCMomentsTailPickerSelection(self.postSessionMode);
...
- (void)updateSearchResultsForSearchController:(UISearchController *)searchController {
    ...
    if (query.length == 0) {
        ...
        self.displayedEntries = [NeoWCMomentsTailEntries() filteredArrayUsingPredicate:predicate];
    }
...
- (void)applyAppID:(NSString *)appID {
    if (self.postSessionMode) {
        @synchronized (NeoWCMomentsTailStateLock()) {
            NeoWCMomentsTailPostSessionSet = YES;
            NeoWCMomentsTailPostSessionAppID = [appID copy];
        }
    } else {
        NSUserDefaults *defaults = NSUserDefaults.standardUserDefaults;
        if (appID.length > 0) [defaults setObject:appID forKey:NeoWCMomentsTailAppIDKey];
        else [defaults removeObjectForKey:NeoWCMomentsTailAppIDKey];
    }
    [self.tableView reloadData];
...
- (void)presentCustomInput {
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"使用" style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
        ...
        if (NeoWCMomentsTailNameForAppID(appID).length > 0) {
...
UIViewController *NeoWCMomentsTailPicker(BOOL postSession, void (^completion)(void)) {
    NeoWCMomentsTailPickerViewController *picker = [NeoWCMomentsTailPickerViewController new];
...
typedef id (*NeoWCMomentsTailObjectIMP)(id, SEL);
typedef id (*NeoWCMomentsTailObjectObjectIMP)(id, SEL, id);
typedef id (*NeoWCMomentsTailObjectObjectObjectIMP)(id, SEL, id, id);
typedef void (*NeoWCMomentsTailVoidIMP)(id, SEL);
typedef void (*NeoWCMomentsTailVoidBoolIMP)(id, SEL, BOOL);

static NeoWCMomentsTailObjectIMP NeoWCMomentsTailOriginalAppInfo;
static NeoWCMomentsTailObjectIMP NeoWCMomentsTailOriginalInit;
static NeoWCMomentsTailObjectObjectObjectIMP NeoWCMomentsTailOriginalInitImagesContacts;
static NeoWCMomentsTailObjectObjectIMP NeoWCMomentsTailOriginalInitSightDraft;
static NeoWCMomentsTailObjectIMP NeoWCMomentsTailOriginalInitTextType;
static NeoWCMomentsTailVoidIMP NeoWCMomentsTailOriginalReloadData;
static NeoWCMomentsTailVoidBoolIMP NeoWCMomentsTailOriginalViewDidAppear;

static id NeoWCMomentsTailAppInfo(id self, SEL selector) {
    NSString *appID = NeoWCMomentsTailEffectiveAppID();
    NSString *appName = NeoWCMomentsTailNameForAppID(appID);
    id info = appID.length > 0 && appName.length > 0
        ? NeoWCPrivateCreateMomentsAppInfo(appID, appName) : nil;
    return info ?: (NeoWCMomentsTailOriginalAppInfo ? NeoWCMomentsTailOriginalAppInfo(self, selector) : nil);
}

static id NeoWCMomentsTailInit(id self, SEL selector) {
    id result = NeoWCMomentsTailOriginalInit ? NeoWCMomentsTailOriginalInit(self, selector) : self;
    NeoWCMomentsTailResetPostSession();
    return result;
}

static id NeoWCMomentsTailInitImagesContacts(id self, SEL selector, id images, id contacts) {
    id result = NeoWCMomentsTailOriginalInitImagesContacts
        ? NeoWCMomentsTailOriginalInitImagesContacts(self, selector, images, contacts) : self;
    NeoWCMomentsTailResetPostSession();
    return result;
}

static id NeoWCMomentsTailInitSightDraft(id self, SEL selector, id draft) {
    id result = NeoWCMomentsTailOriginalInitSightDraft
        ? NeoWCMomentsTailOriginalInitSightDraft(self, selector, draft) : self;
    NeoWCMomentsTailResetPostSession();
    return result;
}

static id NeoWCMomentsTailInitTextType(id self, SEL selector) {
    id result = NeoWCMomentsTailOriginalInitTextType
        ? NeoWCMomentsTailOriginalInitTextType(self, selector) : self;
    NeoWCMomentsTailResetPostSession();
    return result;
}
...
static NSString *NeoWCMomentsTailRightValue(void) {
    NSString *appID = NeoWCMomentsTailEffectiveAppID();
    return NeoWCMomentsTailNameForAppID(appID) ?: @"无小尾巴";
}

static void NeoWCMomentsTailInjectCell(id controller) {
    if (!NeoWCEnhancementEnabled(NeoWCMomentsTailEnabledKey)) return;
    NeoWCPrivateInstallMomentsTailCell(controller,
                                      NSSelectorFromString(@"neowc_onMomentsTailCell"),
                                      NeoWCMomentsTailRightValue());
}

static void NeoWCMomentsTailOpenPicker(id self, SEL selector) {
    NeoWCPrivateResignMomentsComposerInput(self);
    __weak id weakController = self;
    UIViewController *picker = NeoWCMomentsTailPicker(YES, ^{
        id controller = weakController;
        NeoWCPrivateReloadMomentsTailCell(controller, NeoWCMomentsTailRightValue());
    });
    UINavigationController *navigation = [[UINavigationController alloc] initWithRootViewController:picker];
    [(UIViewController *)self presentViewController:navigation animated:YES completion:nil];
}

static void NeoWCMomentsTailReloadData(id self, SEL selector) {
    if (NeoWCMomentsTailOriginalReloadData) NeoWCMomentsTailOriginalReloadData(self, selector);
    NeoWCMomentsTailInjectCell(self);
}

static void NeoWCMomentsTailViewDidAppear(id self, SEL selector, BOOL animated) {
    if (NeoWCMomentsTailOriginalViewDidAppear) NeoWCMomentsTailOriginalViewDidAppear(self, selector, animated);
    NeoWCMomentsTailInjectCell(self);
}

#pragma mark - Hook Registration

static BOOL NeoWCMomentsTailMethodMatches(Class cls, SEL selector, unsigned int arguments,
...
void NeoWCMomentsTailInstallHooks(void) {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        Class uploadClass = NSClassFromString(@"WCUploadTask");
        SEL appInfoSelector = NSSelectorFromString(@"appInfo");
        if (NeoWCMomentsTailMethodMatches(uploadClass, appInfoSelector, 2, '@', "")) {
            IMP original = NULL;
            MSHookMessageEx(uploadClass, appInfoSelector, (IMP)NeoWCMomentsTailAppInfo, &original);
            NeoWCMomentsTailOriginalAppInfo = (NeoWCMomentsTailObjectIMP)original;
        }

        Class composerClass = NSClassFromString(@"WCNewCommitViewController");
        if (!composerClass) return;
        class_addMethod(composerClass, NSSelectorFromString(@"neowc_onMomentsTailCell"),
                        (IMP)NeoWCMomentsTailOpenPicker, "v@:");

#define NEOWC_HOOK_TAIL(name, args, ret, types, replacement, storage, castType) do { \
    SEL s = NSSelectorFromString(name); \
    if (NeoWCMomentsTailMethodMatches(composerClass, s, args, ret, types)) { \
        IMP original = NULL; MSHookMessageEx(composerClass, s, (IMP)replacement, &original); \
        storage = (castType)original; \
    } \
} while (0)
        NEOWC_HOOK_TAIL(@"init", 2, '@', "", NeoWCMomentsTailInit,
                        NeoWCMomentsTailOriginalInit, NeoWCMomentsTailObjectIMP);
        NEOWC_HOOK_TAIL(@"initWithImages:contacts:", 4, '@', "@@", NeoWCMomentsTailInitImagesContacts,
                        NeoWCMomentsTailOriginalInitImagesContacts, NeoWCMomentsTailObjectObjectObjectIMP);
        NEOWC_HOOK_TAIL(@"initWithSightDraft:", 3, '@', "@", NeoWCMomentsTailInitSightDraft,
                        NeoWCMomentsTailOriginalInitSightDraft, NeoWCMomentsTailObjectObjectIMP);
        NEOWC_HOOK_TAIL(@"initWithTextType", 2, '@', "", NeoWCMomentsTailInitTextType,
                        NeoWCMomentsTailOriginalInitTextType, NeoWCMomentsTailObjectIMP);
        NEOWC_HOOK_TAIL(@"reloadData", 2, 'v', "", NeoWCMomentsTailReloadData,
                        NeoWCMomentsTailOriginalReloadData, NeoWCMomentsTailVoidIMP);
        NEOWC_HOOK_TAIL(@"viewDidAppear:", 3, 'v', "B", NeoWCMomentsTailViewDidAppear,
                        NeoWCMomentsTailOriginalViewDidAppear, NeoWCMomentsTailVoidBoolIMP);
#undef NEOWC_HOOK_TAIL
        NeoWCLog(@"朋友圈来源尾巴 Hook 已安装");
    });
}
