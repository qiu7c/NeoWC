...
- 聊天转场性能修复：输入框左右滑不再运行于 `layoutSubviews`；防撤回消息 Cell 不再投递跨生命周期主队列任务，非气泡旁模式不额外请求布局；提示设置变化只标记可见 Cell 待布局，不再递归强制 `layoutIfNeeded`；系统提示颜色只在实际变化时重绘；消息、步数与广告兼容性标记改为每个入口仅记录一次。
