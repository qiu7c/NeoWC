...
14. 开启“消息时间显示”，确认“头像下方”和“消息旁”严格二选一；头像模式的时间固定紧贴当前头像底部，表情或大图消息不得把时间推到远处。消息旁模式分别验证文字、图片、语音、表情、文件和引用消息，并确认状态更新与滚动不卡顿；其内容定位只允许私有字段/Selector 和最多两层的有界候选查找，禁止恢复整棵视图树递归扫描。切换顶部/中间/底部位置并与防撤回提示同时测试，默认底部不得与居中提示重叠；修改颜色和透明度后可立即作用于可见消息。
