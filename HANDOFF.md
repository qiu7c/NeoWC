...
- 图片编辑快捷发送已能从 `EditImageAttr setEditedImage:` 取得最终编辑图；确认页遍历所有前台 Window（包括微信实际承载页面的 `iConsoleWindow`），直接使用当前编辑页或页卡 Controller 承载微信官方确认页，不要求先返回聊天 Controller。仅在页面正切换、完全没有可见 Controller 时最多重试约 3.6 秒。
