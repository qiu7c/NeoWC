...
2. 当前未提交修复
   - 聊天搜索已补齐 WCPulse/WeChatX 的控制器归一化、导航前处理及 `object_getIvar` 内部目标解析：从 Helper 的对象 ivar 中寻找真正实现搜索跳转的内部目标，对 Helper、内部目标和搜索控制器开启侧滑取消及原生 `interactivePopGestureRecognizer`；聊天页重新出现时先调用内部 `msgSearchBarCancel` 清理残留搜索栏，再恢复胶囊顶栏。
