...
- 长文本截图不再使用 `drawViewHierarchyInRect:` 渲染消息 Cell；该 API 会按当前屏幕渲染表面产生横向瓦片断层。消息 Cell 改为完整 `CALayer renderInContext:`，继续保留气泡背景兜底与 0.1.0 的单 Cell 合成方式。
- 设置分类重新整理：小游戏结果、图片编辑快捷发送、长截图和多选导出归入“聊天增强”；登录授权、朋友圈、运动和广告归入“常用增强”；新增“界面优化”。界面优化提供聊天输入栏总开关以及独立的输入框内部圆角、外部 `InputToolViewBar` 圆角，运行时保存并可恢复微信原始 layer 状态；插件显示管理也移至该分类。
...
- 长截图二次修复：取消范围选择页，退出多选并确认 `isInMultiSelectMsg/isMultiSelectMode` 均结束后直接生成；长消息使用 `CALayer renderInContext:` 离屏渲染，避免 44/97.6667 pt 顶栏可视边界形成横线；消息之间增加 8 pt 间距。
