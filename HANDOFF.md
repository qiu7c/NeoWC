...
- 最近已推送提交：`ca25a4e Fix voice message repeat upload flow`。
- 当前未提交修改完整删除了 wxid 网络验证与授权系统：移除普通授权、远程黑名单、管理员识别、授权管理页、网络接口、授权缓存驱动的页面刷新及限制弹窗。所有用户均直接使用本地业务功能，设置首页个人信息区只显示头像、昵称和 wxid。
...
| `Sources/NeoWCInterfaceTweaks.m` | 全局头像圆角、输入栏圆角、免打扰图标隐藏与原状态恢复 |
...
- 全局头像圆角：只覆盖 `MMHeadImageView` / `FakeHeadImageView`，支持 0–100% 圆角程度并保留关闭前的微信原始层状态
...
2. 当前未提交修复
   ...
   - beta47 完整删除朋友圈转发按钮长按发送指定好友：移除联系人选择控制器、长按手势、好友消息组装、转发会话和两个运行时处理方法，只保留短按进入朋友圈发布页。
   - beta50 明确插件管理页列表卡片层级：插件名称、版本、箭头或开关所在区域使用 `systemBackgroundColor` 连成一张圆角分组卡片，首尾行分别裁切连续圆角，选中态使用系统填充色；继续保持无分割线并自动适配深色模式。
   - beta51 将插件管理默认分类精简为“定制 / 功能”，首次进入默认显示“定制”；未被用户手动归类时，Controller 为 `NeoWCSettingsViewController` 的 NeoWC 自动进入“定制”，其他注册插件自动进入“功能”。已有有效手动分类映射继续优先，清空配置后恢复上述默认规则。
   - beta52 将插件管理顶部图标从 88 点缩小为 72 点并压缩页眉高度；默认改用与插件入口相同的微信原生图标。顶部图标现在可独立选择微信原生、灯泡、拼图、印章、宫格或相册自定义，内置选项与入口图标共用同一图标表但分别持久化，清空配置时一并恢复默认。
   - beta53 修复插件列表点击 Controller 后停留在原页的问题：列表内部页面跳转与插件入口统一优先调用微信导航控制器的 `PushViewController:animated:`，仅在该入口不存在时回退 UIKit `pushViewController:animated:`；入栈前记录目标 Controller 与导航类。
   - beta54 收紧插件管理页的信息密度：插件行由 62 点降至 56 点，页眉标题、副标题、分类切换器及插件名称/版本文字统一缩小一档并继续通过 `UIFontMetrics` 支持动态字体。插件列表卡片在浅色模式下固定为纯白，深色模式使用系统深色分组背景，避免浅色页面出现灰色卡片。
   - beta56 将账号身份获取改为完全被动缓存：仅在微信自身调用 `CContactMgr getSelfContact` 时记录 wxid、昵称和头像地址，设置页及个人信息页眉均不再主动访问联系人私有服务；头像改用普通 `NSURLSession` 异步加载。
   ...
   - 正式版本更新为 0.1.3；用户首次进入该版本的 NeoWC 设置首页时显示一次更新日志，使用版本化 Key 记录已展示状态，同版本后续进入不重复弹出。
   - 0.1.3 后续完整删除 wxid 网络验证与授权系统：授权/黑名单请求、管理员哈希、授权管理页、状态缓存、通知刷新、限制弹窗及功能门禁均不再参与运行。
   ...
   - 0.1.3 后续按 WCPulse 的导航手势仲裁方式修复消息右滑与返回冲突：消息 Pan 每次通过代理动态取得当前 `interactivePopGestureRecognizer` 及可能存在的全屏返回手势并主动让其优先；同时记录手势最初触点，导航栈可返回时屏幕左侧 50 点区域完全保留给返回，避免触点移动进 Cell 后误触复制、复读等右滑动作。
...
10. 在“我”页确认只出现一个“插件”入口；验证 Controller 与 Switch 注册、默认“定制/功能”分类（NeoWC 自动在定制，其他插件自动在功能）、分类增删改、改名/版本、收纳恢复、拖动及序号排序、分页、顶部设置和清空配置。若设备仍安装原 `storage.dylib`，必须先移除，避免同名 `WCPluginsMgr` / `WCPluginsViewController` Runtime 类冲突。
11. 开启“消息手势”，分别用自己和对方的文字、图片、语音消息验证左右滑、双击、三击；再从屏幕左缘执行系统侧滑返回，确认返回手势优先且不会误触发右滑“复制”等消息动作。确认对方撤回不可选，删除/撤回仍走微信确认流程，文字和微信允许转发的媒体可复读，语音复读会生成一条新的可播放语音且不会改动原消息。
12. 在联网、断网和切换账号状态下重复进入 NeoWC 设置页及前后台切换，确认页面不发起任何 wxid 授权/黑名单请求，不显示授权状态、限制弹窗或授权管理入口，也不会因网络状态卡住；所有本地功能保持可用。
13. 确认设置首页不再存在功能搜索按钮、搜索框或搜索结果构建逻辑。
14. 开启“消息时间显示”，确认“头像下方”和“消息旁”严格二选一；头像模式的时间固定紧贴当前头像底部，表情或大图消息不得把时间推到远处，首次显示及 Cell 复用后均需在三次弱引用合并刷新内恢复；若微信重建子视图移除了标签，复用关联对象时必须重新挂回 Cell。消息旁模式必须与防撤回旁提示共用 `NeoWCMessageSideAnchorView`，只允许 `getBgImageView`、内容字段和明确 Selector，禁止任何视图树候选扫描；分别验证文字、图片、语音、表情、文件和引用消息，并确认状态更新与滚动不卡顿。切换顶部/中间/底部位置并与防撤回提示同时测试，默认底部不得与居中提示重叠；修改颜色和透明度后可立即作用于可见消息。
...
## 14. 2026-08-20 网络验证移除与消息时间调整

- wxid 网络验证与整套授权系统已完整删除，不再有授权请求、黑名单检查、管理员入口、授权管理页、状态通知、限制弹窗或功能门禁。
- 头像下方消息时间新增 `com.qiu7c.neowc.chat.message-time.avatar-spacing`，默认 `0`，设置页限制为 `-6...8`；负值向上、正值向下。
...
最近已推送提交为 ca25a4e。当前未提交修改完整删除了 wxid 网络验证与授权系统；设置首页个人信息区只保留头像、昵称和 wxid，所有本地功能不再受网络、授权或黑名单状态影响。HANDOFF.md 已更新，必须保留这些修改。
