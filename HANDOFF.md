...
2. 当前未提交修复
   - beta26 已按要求完整删除聊天搜索、关键词提醒和群聊艾特提醒，包括设置 Key、设置项、运行时处理、边缘提示 UI、搜索控制器 Hook 与兼容性登记；同时移除删除提醒模块后遗留的未使用 `NeoWCContactManager`，修复 Theos `-Werror,-Wunused-function` 构建失败。
...
1. 编译并安装 beta26，确认设置页、聊天页和运行时均不再出现搜索、关键词提醒或群聊艾特提醒。
