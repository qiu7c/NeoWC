...
2. 当前未提交修复
   ...
   - beta53 修复插件列表点击 Controller 后停留在原页的问题：列表内部页面跳转与插件入口统一优先调用微信导航控制器的 `PushViewController:animated:`，仅在该入口不存在时回退 UIKit `pushViewController:animated:`；入栈前记录目标 Controller 与导航类，授权请求仍只在 NeoWC 页面完成显示后异步开始。
...
13. 重复进入 NeoWC 设置页，确认本地 wxid、昵称、头像和授权状态首帧直接显示，12 小时内不会因再次联网校验而跳成“检测中”或整页刷新；普通授权后台失败时保留上次状态，黑名单变化仍立即生效。确认设置首页不再存在功能搜索按钮、搜索框或搜索结果构建逻辑。
...
## 14. 2026-08-16 授权与消息时间调整

- 黑名单网络检测只允许由 NeoWC 根设置页首次显示时触发；启动、回前台、聊天、朋友圈、调试入口及通用增强 Hook 不再查询或读取黑名单状态。
- 接口明确返回黑名单后，先将 `com.qiu7c.neowc.enabled` 写为 `NO`，再结束当前进程；普通授权状态仍只用于设置页展示。
